...
class GetEventsSchema(BaseModel):
    ...
    start_datetime: str = Field(
        ...
        description=(
            ...
            f"If the date is set to be today then use {datetime.now().strftime('%Y-%m-%dT%H:%M:%S')}"
        )
    )
    end_datetime: str = Field(
        ...
        description=(
            ...
            " 2023, at 10:30 AM"
            "Do not include timezone info as it will be automatically processed."
            f"If the date is set to be today then use {(datetime.now() + timedelta(days=7)).strftime('%Y-%m-%dT%H:%M:%S')}"
        )
    )
...
if __name__ == "__main__":
    ...
    tool_res = createeventtool.run(tool_input={
        "start_datetime": "2024-02-18T10:30:00",
        "end_datetime": "2024-02-18T11:30:00", 
        "summary": "Test event"
        })
    print(tool_res)
